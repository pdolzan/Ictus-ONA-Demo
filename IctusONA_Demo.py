...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    [ "Hierarhija","Vprašalnik", "Matrike", "Omrežje", "ONA Aplikacije","Ictus Vizitka"])
...
with tab3:
   ...
   "Saj je za ostale zaupanje vreden vir informacij in ga obravnavajo kot prijatelja.")
    ...
    if all(k in st.session_state for k in ["trust", "advice", "friends"]):
        ...
        trust_matrix = create_matrix(st.session_state["trust"], density=0.15, reciprocity_rate=0.2)
        advice_matrix = create_matrix(st.session_state["advice"], density=0.15, reciprocity_rate=0.2)
        friends_matrix = create_matrix(st.session_state["friends"], density=0.08, reciprocity_rate=0.6)
        ...
        combined_matrix = trust_matrix + advice_matrix + friends_matrix
...
with tab4: 
    ...
    if all(k in st.session_state for k in ["trust", "advice", "friends"]):
        matrices = {
            "Zaupanje": trust_matrix,
            "Svetovanje": advice_matrix,
            "Prijateljstvo": friends_matrix,
            "Skupno": combined_matrix
        }
...
with tab6:
    ...
    # Display the image
    st.image(str(ASSETS/"img1.png"), use_container_width=True)
    st.write("")
    st.markdown("<hr style='border: 1px dashed  #fac108; margin: 1rem 0;'>", unsafe_allow_html=True)
    ...
    st.markdown("<hr style='border: 1px dashed  #fac108; margin: 1rem 0;'>", unsafe_allow_html=True)
    st.write("")
    st.image(str(ASSETS/"img2.png"), use_container_width=True)